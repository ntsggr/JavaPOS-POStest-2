...
public class FirmwareUpdateDlg extends JDialog implements ActionListener, StatusUpdateListener {
	...
	public FirmwareUpdateDlg(Object object) throws Exception {
		...
		if (compareMethod == null || updateMethod == null || getCapUpdateFirmware == null
				|| getCapCompareFirmwareVersion == null) {
			throw new Exception(
					"Object passed to FirmwareUpdateDialog() does not support the firmware interface.\nPerhaps the Service Object is not UPOS v1.9 complient.");
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"call to a getCap function threw an IllegalAccessException:\n" + e.getMessage(),
					"Exception", JOptionPane.ERROR_MESSAGE);
		}
		...
		try {
			...
			JposException je = (JposException) e.getTargetException();
			JOptionPane.showMessageDialog(null,
					"exception calling addStatusUpdateListener:\n" + je.getMessage(), "Exception",
				...
				try {
					...
					try {
						if (e.getClass().equals(Class.forName("JposException"))) {
							JOptionPane.showMessageDialog(
									null,
									"call to compareFirmwareVersion threw a JposException:\n"
											+ e.getMessage(), "Exception", JOptionPane.ERROR_MESSAGE);
						}
					} catch (ClassNotFoundException ex) {
						JOptionPane.showMessageDialog(null,
								"Exception in compareFirmwareVersion:\n" + e.getMessage(), "Exception",
								JOptionPane.ERROR_MESSAGE);
					}
				}
			} else if (updateRadio.isSelected()) {
				messageWindow
						.setText(messageWindow.getText() + "The update firmware process is starting..\n");
				goButton.setEnabled(false);
	...
	private class UpdateThread extends Thread {
		public void run() {
			try {
				...
				JOptionPane.showMessageDialog(null,
						"call to updateFirmware threw an IllegalAccessException:\n" + e.getMessage(),
						"Exception", JOptionPane.ERROR_MESSAGE);
			} catch (Exception e) {
				try {
					if (e.getClass().equals(Class.forName("JposException"))) {
